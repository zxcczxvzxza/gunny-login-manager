...
// kernel32: dùng để ép console về UTF-8, tránh log tiếng Việt/emoji bị vỡ
// thành ký tự lạ (mojibake) khi chạy trong terminal Windows (cp mặc định 437/1258).
const kernel32 = koffi.load('kernel32.dll');
const SetConsoleOutputCP = kernel32.func('bool __stdcall SetConsoleOutputCP(uint32 wCodePageID)');
const SetConsoleCP = kernel32.func('bool __stdcall SetConsoleCP(uint32 wCodePageID)');

/**
 * Chuyển code page của console sang UTF-8 (65001). Gọi sớm lúc app khởi động.
 * An toàn khi không có console (packaged) — chỉ trả về false.
 */
export function setConsoleUtf8() {
  try {
    ...
    if (IsWindowVisible(hwnd)) {
      ...
      if (titleLen > 0) {
        ...
        if (title.toLowerCase().includes(lowerSubstring)) {
          ...
          console.log(
            `[AutoClose] Found alert window: "${title}" (Class: ${className}), closing...`
          );
