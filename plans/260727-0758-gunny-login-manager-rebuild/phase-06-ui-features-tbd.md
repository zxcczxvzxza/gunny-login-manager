...
- **Feat #3 — Cập nhật game (PR #11 → #13):** ban đầu uỷ quyền mở `GunnyClient.exe` (PR #11); user không muốn cửa sổ launcher bật lên. Đã xác minh `resource/` ghi được không cần admin + protocol (Hash = MD5, file tải từ `res.gnddt.com`), nên chuyển sang **tự tải trong app** (PR #13): `updateService.updateGameResources()` tải `ResourceInfo.xml` → so MD5 với `resource/` local → tải file thiếu/khác, có tiến trình + nút Dừng. Bỏ `launcherService`/`gunnyLauncherPath`; thêm settings `resourceManifestUrl`/`resourceBaseUrl`. IPC `game:update-resources`/`game:stop-update`. Verify e2e server thật: checked 443, downloaded 0.
